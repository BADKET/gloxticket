    ...
    async execute(interaction) {
        ...
        if (interaction.isButton()) {
            if (interaction.customId === "openTicket") {
                ...
                const embed = new discord_js_1.EmbedBuilder()
                    ...
                    .setTitle(this.client.locales.getSubValue("other", "selectTicketTypePlaceholder"))
                    .setDescription("Please select a category from the menu below to open your ticket.");
                interaction
                    .editReply({
                    embeds: [embed],
                    components: [row],
            ...
            if (interaction.customId === "close") {
                await interaction.deferReply({ ephemeral: true }).catch((e) => console.log(e));
                (0, close_1.close)(interaction, this.client, this.client.locales.getSubValue("other", "noReasonGiven"), this.client.config.closeOption.deleteTicket);
            }
            if (interaction.customId === "close_askReason") {
                (0, close_askReason_1.closeAskReason)(interaction, this.client, this.client.config.closeOption.deleteTicket);
            }
        ...
        if (interaction.isStringSelectMenu()) {
            ...
            if (interaction.customId === "removeUser") {
                ...
                await this.client.prisma.tickets.update({
                    data: {
                        invited: JSON.stringify(JSON.parse(ticket?.invited ?? "[]").filter((userid) => interaction.values.find((rUID) => rUID === userid) === undefined)),
                    },
                    where: {
        ...
        if (interaction.isModalSubmit()) {
            ...
            if (interaction.customId === "askReasonClose") {
                ...
                await interaction.deferReply().catch((e) => console.log(e));
                (0, close_1.close)(interaction, this.client, interaction.fields.fields.first()?.value, true);
            }
        }
    }
}
